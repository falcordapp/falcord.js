...
test('Unauthorized', () => {
	const error = new DiscordAPIError(
		...
		401,
		'PATCH',
		'https://falcord.ixchats.com/api/v10/guilds/:id',
		{
			files: undefined,
			body: undefined,
		},
	...
	expect(error.url).toEqual('https://falcord.ixchats.com/api/v10/guilds/:id');
...
test('Invalid Form Body Error (error.{property}._errors.{index})', () => {
	const error = new DiscordAPIError(
		...
		400,
		'PATCH',
		'https://falcord.ixchats.com/api/v10/users/@me',
		{
			files: undefined,
			body: {
				username: 'a',
	...
	expect(error.url).toEqual('https://falcord.ixchats.com/api/v10/users/@me');
	expect(error.requestBody.files).toBeUndefined();
...
test('Invalid FormFields Error (error.errors.{property}.{property}.{index}.{property}._errors.{index})', () => {
	const error = new DiscordAPIError(
		...
		400,
		'POST',
		'https://falcord.ixchats.com/api/v10/channels/:id',
		{},
	);
	...
	expect(error.name).toEqual('DiscordAPIError[50035]');
	expect(error.status).toEqual(400);
	expect(error.url).toEqual('https://falcord.ixchats.com/api/v10/channels/:id');
});
...
test('Invalid Oauth Code Error (error.error)', () => {
	const error = new DiscordAPIError(
		...
		400,
		'POST',
		'https://falcord.ixchats.com/api/v10/oauth2/token',
		{
	...
	expect(error.status).toEqual(400);
	expect(error.url).toEqual('https://falcord.ixchats.com/api/v10/oauth2/token');
});
