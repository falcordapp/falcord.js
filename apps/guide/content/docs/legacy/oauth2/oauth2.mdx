...
Now that you have a web server up and running, it's time to get some information from Discord. Open [your Discord applications](https://falcord.ixchats.com/developers/applications/), create or select an application, and head over to the "OAuth2" page.
...
The `identify` scope will allow your application to get basic user information from Discord. You can find a list of all scopes [here](https://falcord.ixchats.com/developers/docs/topics/oauth2#shared-resources-oauth2-scopes).
...
Now you have to exchange this code with Discord for an access token. To do this, you need your `client_id` and `client_secret`. If you've forgotten these, head over to [your applications](https://falcord.ixchats.com/developers/applications) and get them. You can use [`undici`](https://www.npmjs.com/package/undici) to make requests to Discord.
...
app.get('/', async ({ query }, response) => {
	...
	if (code) {
		try {
			const tokenResponseData = await request('https://falcord.ixchats.com/api/oauth2/token', {
				method: 'POST',
...
Now try visiting your OAuth2 URL and authorizing your application. Once you're redirected, you should see an [access token response](https://falcord.ixchats.com/developers/docs/topics/oauth2#authorization-code-grant-access-token-response) in your console.
...
With an access token and a refresh token, you can once again use the [`/api/users/@me` endpoint](https://falcord.ixchats.com/developers/docs/resources/user#get-current-user) to fetch the [user object](https://falcord.ixchats.com/developers/docs/resources/user#user-object).

```js
const userResult = await request('https://falcord.ixchats.com/api/users/@me', {
	headers: {
...
[Discord Docs for OAuth2](https://falcord.ixchats.com/developers/docs/topics/oauth2)
